...
window.openIomConfigModal = function() {
  const pass = prompt("Enter Password to Edit IOM Configuration:");
  if (pass !== "a") {
    if (pass !== null) alert("Incorrect Password!");
    return;
  }
  
  const existing = document.getElementById('iom-config-modal');
  if (existing) existing.remove();
  ...
  let tempStaffList = JSON.parse(JSON.stringify(IOM_STAFF_LIST));
  let tempIsLocked = isLocked;

  const modalHTML = `
    <div id="iom-config-modal" style="position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(15, 23, 42, 0.7); backdrop-filter:blur(10px); z-index:10000; display:flex; justify-content:center; align-items:center;">
      <div class="glass-card" style="width:95%; max-width:900px; max-height:90vh; display:flex; flex-direction:column; padding:2rem; background:#fff; border-radius:12px; overflow:hidden;">
        
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
          <h3 style="color:#0f172a; font-size:1.5rem; margin:0; font-family:'Inter', sans-serif;">IOM Configuration</h3>
          <div>
            <label style="margin-right:1rem; font-weight:bold; color:#333; cursor:pointer;">
              <input type="checkbox" id="iom-lock-toggle" ${tempIsLocked ? 'checked' : ''} style="margin-right:0.5rem; transform:scale(1.2);">
              Lock IOM Inputs
            </label>
            <button onclick="document.getElementById('iom-config-modal').remove()" style="padding:0.5rem 1rem; background:#f1f5f9; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">Cancel</button>
            <button id="save-iom-config-btn" style="padding:0.5rem 1rem; background:#2563eb; color:#fff; border:none; border-radius:6px; cursor:pointer; font-weight:bold; margin-left:0.5rem;">Save Changes</button>
          </div>
        </div>
          ...
          <table style="width:100%; border-collapse:collapse; text-align:left;">
            <thead style="background:#f8fafc; position:sticky; top:0; z-index:1; box-shadow:0 1px 2px rgba(0,0,0,0.05);">
              <tr>
                ...
                <th style="padding:0.75rem; border-bottom:1px solid #e2e8f0;">Designation</th>
                <th style="padding:0.75rem; border-bottom:1px solid #e2e8f0;">Department</th>
                <th style="padding:0.75rem; border-bottom:1px solid #e2e8f0; text-align:center;">Action</th>
              </tr>
            </thead>
            <tbody id="iom-config-tbody">
              <!-- Rows injected here -->
            </tbody>
          </table>
        </div>
        ...
        <div style="margin-top:1rem;">
          <button id="iom-add-row-btn" style="padding:0.5rem 1rem; background:#10b981; color:#fff; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">+ Add New Staff Row</button>
        </div>
      </div>
    </div>
  `;
  ...
  const tbody = document.getElementById('iom-config-tbody');
  
  function renderRows() {
    tbody.innerHTML = '';
    tempStaffList.forEach((staff, index) => {
      if (!staff) return;
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td style="padding:0.5rem; border-bottom:1px solid #e2e8f0;"><input type="text" class="conf-id" value="${staff.id || ''}" style="width:100%; padding:0.4rem; border:1px solid #cbd5e1; border-radius:4px;"></td>
        <td style="padding:0.5rem; border-bottom:1px solid #e2e8f0;"><input type="text" class="conf-name" value="${staff.name || ''}" style="width:100%; padding:0.4rem; border:1px solid #cbd5e1; border-radius:4px;"></td>
        <td style="padding:0.5rem; border-bottom:1px solid #e2e8f0;"><input type="text" class="conf-desig" value="${staff.designation || ''}" style="width:100%; padding:0.4rem; border:1px solid #cbd5e1; border-radius:4px;"></td>
        <td style="padding:0.5rem; border-bottom:1px solid #e2e8f0;"><input type="text" class="conf-dept" value="${staff.department || ''}" style="width:100%; padding:0.4rem; border:1px solid #cbd5e1; border-radius:4px;"></td>
        <td style="padding:0.5rem; border-bottom:1px solid #e2e8f0; text-align:center;">
          <button class="delete-row-btn" data-index="${index}" style="padding:0.3rem 0.6rem; background:#ef4444; color:#fff; border:none; border-radius:4px; cursor:pointer;">Delete</button>
        </td>
      `;
  ...
  document.getElementById('iom-add-row-btn').addEventListener('click', () => {
    tempStaffList.push({ id: '', name: '', designation: '', department: '' });
    renderRows();
    // Scroll to bottom
  ...
  document.getElementById('save-iom-config-btn').addEventListener('click', () => {
    // Collect data
    const rows = tbody.querySelectorAll('tr');
    const newList = [];
    rows.forEach(tr => {
      const id = tr.querySelector('.conf-id').value.trim();
      const name = tr.querySelector('.conf-name').value.trim();
      const desig = tr.querySelector('.conf-desig').value.trim();
      const dept = tr.querySelector('.conf-dept').value.trim();
      if (id || name) {
        newList.push({ id, name, designation: desig, department: dept });
      }
    });

    const lockChecked = document.getElementById('iom-lock-toggle').checked;

    IOM_STAFF_LIST = newList;
