    ...
    <div class="ios-hm-card" role="dialog" aria-labelledby="ios-hm-title">
      ...
      <div class="ios-hm-header">
        ...
        <div class="ios-hm-header-actions">
          ...
          <button id="fan-merge-history-btn" class="ios-hm-merge-btn" onclick="window.showFanAssembleDimmerMergedHistory()" type="button" title="Fan Assemble + Dimmer Merged History">
            <span class="ios-hm-merge-icon-wrap" aria-hidden="true">
            ...
            <span class="ios-hm-merge-label">Fan Assemble + Dimmer</span>
          </button>
          <button id="fan-roj-shapla-merge-history-btn" class="ios-hm-merge-btn" onclick="window.showFanRojonigondhaShaplaMergedHistory()" type="button" title="Fan Rojonigondha + Shapla Merged History">
            <span class="ios-hm-merge-icon-wrap" aria-hidden="true">
              <svg class="ios-hm-merge-svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><path d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path></svg>
            </span>
      ...
      '<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">' +
        '<div>' +
          '<div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">' +
            ...
            '<span style="font-size:11px; font-weight:800; color:#4f46e5; background:rgba(99,102,241,0.12); padding:4px 12px; border-radius:99px; border:1px solid rgba(99,102,241,0.25);">Fan Assemble + Dimmer</span>' +
          '</div>' +
...
function getAllDatesInMonth(monthKey) {
  ...
  let maxSavedDay = 0;
  if (window.savedHistoryDates) {
    window.savedHistoryDates.forEach(function(dStr) {
      const p = dStr.split('-');
      if (p.length === 3 && parseInt(p[0], 10) === year && parseInt(p[1], 10) === month) {
        const dayVal = parseInt(p[2], 10);
        if (!isNaN(dayVal) && dayVal > maxSavedDay) {
          maxSavedDay = dayVal;
        }
      }
    });
  ...
  let lastDayToInclude = totalDaysInMonth;
  if (isCurrentMonth) {
    const todayDay = now.getDate();
    lastDayToInclude = Math.min(totalDaysInMonth, Math.max(todayDay, maxSavedDay));
  } else if (!isPastMonth) {
    lastDayToInclude = Math.min(totalDaysInMonth, Math.max(1, maxSavedDay));
  }

  const dates = [];
  for (let d = 1; d <= lastDayToInclude; d++) {
...
window.populatePDFMonthDropdown = function() {
  ...
  // Determine target month: current history month if available, otherwise latest month
  let targetMonthKey = sortedMonths[0].info.key;
  if (window.historyCurrentDate instanceof Date && !isNaN(window.historyCurrentDate.getTime())) {
    const curY = window.historyCurrentDate.getFullYear();
    const curM = String(window.historyCurrentDate.getMonth() + 1).padStart(2, '0');
    const curKey = curY + '-' + curM;
    if (monthsMap.has(curKey)) {
      targetMonthKey = curKey;
    }
  }
  ...
  sortedMonths.forEach(function(m) {
    ...
    if (m.info.key === targetMonthKey) {
      opt.selected = true;
    }
    select.appendChild(opt);
  });

  select.value = targetMonthKey;
};
...
window.downloadMonthlyHistoryPDF = function() {
  ...
  const datesInMonth = getAllDatesInMonth(monthKey);
  ...
  const fetchPromises = datesInMonth.map(function(dateStr) {
    return window.firebaseDb.ref('mep_attendance_history/' + dateStr).once('value').then(function(snap) {
      ...
      return {
        dateStr: dateStr,
        hasSnapshot: snap.exists() && val !== null,
        state: val
      };
    }).catch(function() {
      return {
...
function generateAndPrintMonthlyReport(monthKey, results) {
  ...
  const tableRowsHtml = results.map(function(res, index) {
    ...
    const rowBgStyle = isFriday ? 'style="background-color: #fee2e2;"' : '';
  ...
  const avgAuth = daysWithDataCount > 0 ? (totalAuthSum / daysWithDataCount).toFixed(1) : '0.0';
  const avgExist = daysWithDataCount > 0 ? (totalExistSum / daysWithDataCount).toFixed(1) : '0.0';
  const avgPresent = daysWithDataCount > 0 ? (totalPresentSum / daysWithDataCount).toFixed(1) : '0.0';
  const avgAbsent = daysWithDataCount > 0 ? (totalAbsentSum / daysWithDataCount).toFixed(1) : '0.0';
  const overallPct = totalAuthSum > 0 ? Math.round((totalPresentSum / totalAuthSum) * 100) : 0;
...
function _renderHistoryState(dateStr, state, container) {
  try {
    ...
    container.innerHTML =
      '<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; margin-bottom:20px; background:#f8fafc; border:1px solid #cbd5e1; padding:18px 20px; border-radius:16px;">' +
        '<div>' +
          '<div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">' +
            '<h3 style="margin:0; font-size:20px; font-weight:900; color:#0f172a; letter-spacing:-0.02em;">Attendance Snapshot</h3>' +
            '<span style="font-size:11px; font-weight:800; color:#0284c7; background:rgba(2,132,199,0.12); padding:4px 12px; border-radius:99px; border:1px solid rgba(2,132,199,0.25);">Single Date Archive</span>' +
          '</div>' +
          '<div style="font-size:12px; font-weight:600; color:#64748b;">📅 ' + formattedDate + ' • Factory Daily Attendance Data</div>' +
        '</div>' +
        '<div style="display:flex; align-items:center; gap:14px;">' +
          '<div style="text-align:right;">' +
            '<span style="font-size:10px; font-weight:800; color:#475569; text-transform:uppercase; letter-spacing:0.05em; display:block;">Overall Attendance</span>' +
            '<span style="font-size:22px; font-weight:900; color:#059669;">' + pct + '%</span>' +
          '</div>' +
          '<div style="display:flex; gap:6px;">' +
            (window.historyEditMode ?
              '<button id="btn-save-history" onclick="window.saveHistoryChanges(\'' + historyEscapeHtml(dateStr) + '\')" type="button" style="background:#10b981; color:#fff; border:none; border-radius:10px; font-weight:800; padding:8px 16px; cursor:pointer; font-size:12px;">✓ Save</button>' +
              '<button onclick="window.toggleHistoryEditMode()" type="button" style="background:#e2e8f0; color:#475569; border:none; border-radius:10px; font-weight:800; padding:8px 14px; cursor:pointer; font-size:12px;">✕ Cancel</button>'
            :
              '<button onclick="window.toggleHistoryEditMode()" type="button" style="background:linear-gradient(135deg, #3b82f6, #6366f1); color:#fff; border:none; border-radius:10px; font-weight:800; padding:8px 16px; cursor:pointer; font-size:12px; display:inline-flex; align-items:center; gap:6px;">' +
                '<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>' +
                '<span>Edit</span>' +
              '</button>' +
              '<button onclick="window.deleteHistoryDate(\'' + historyEscapeHtml(dateStr) + '\')" type="button" style="background:#fef2f2; color:#dc2626; border:1px solid #fca5a5; padding:8px 14px; border-radius:10px; font-size:12px; font-weight:800; cursor:pointer; display:inline-flex; align-items:center; gap:6px;">' +
                '<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v5"/><path d="M14 11v5"/></svg>' +
                '<span>Delete</span>' +
              '</button>'
            ) +
          '</div>' +
      ...
      '<div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:12px; margin-bottom:20px;">' +
        '<div style="background:#ffffff; border:1px solid #cbd5e1; border-radius:12px; padding:12px 16px; box-shadow:0 2px 6px rgba(15,23,42,0.04);">' +
          '<div style="font-size:11px; font-weight:800; color:#475569; text-transform:uppercase; letter-spacing:0.04em;">Authorized</div>' +
          '<div style="font-size:24px; font-weight:900; color:#6d28d9; margin-top:2px;">' + (totalAuth || totalExist) + '</div>' +
        '</div>' +
        '<div style="background:#ffffff; border:1px solid #cbd5e1; border-radius:12px; padding:12px 16px; box-shadow:0 2px 6px rgba(15,23,42,0.04);">' +
          '<div style="font-size:11px; font-weight:800; color:#475569; text-transform:uppercase; letter-spacing:0.04em;">Existing</div>' +
          '<div style="font-size:24px; font-weight:900; color:#2563eb; margin-top:2px;">' + totalExist + '</div>' +
        '</div>' +
        '<div style="background:#ffffff; border:1px solid #86efac; border-radius:12px; padding:12px 16px; box-shadow:0 2px 6px rgba(15,23,42,0.04);">' +
          '<div style="font-size:11px; font-weight:800; color:#047857; text-transform:uppercase; letter-spacing:0.04em;">Present</div>' +
          '<div style="font-size:24px; font-weight:900; color:#059669; margin-top:2px;">' + totalPresent + '</div>' +
        '</div>' +
        '<div style="background:#ffffff; border:1px solid ' + (totalAbsent > 0 ? '#fca5a5' : '#cbd5e1') + '; border-radius:12px; padding:12px 16px; box-shadow:0 2px 6px rgba(15,23,42,0.04);">' +
          '<div style="font-size:11px; font-weight:800; color:' + (totalAbsent > 0 ? '#b91c1c' : '#475569') + '; text-transform:uppercase; letter-spacing:0.04em;">Absent</div>' +
          '<div style="font-size:24px; font-weight:900; color:' + (totalAbsent > 0 ? '#dc2626' : '#64748b') + '; margin-top:2px;">' + totalAbsent + '</div>' +
        '</div>' +
      '</div>' +
