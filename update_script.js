...
// Helper to log if nothing was replaced
let initialContent = content;

// 1. Add Button
content = content.replace(
  /(<button id="fan-merge-history-btn"[\s\S]*?<\/button>)/,
  `$1\n          <button id="fan-roj-shapla-merge-history-btn" class="ios-hm-merge-btn" style="margin-left: 4px;" onclick="window.showFanRojonigondhaShaplaMergedHistory()" type="button">
            <span class="ios-hm-merge-icon" aria-hidden="true">↔</span>
            <span>Rojonigondha + Shapla</span>
          </button>`
);

// 2. update window._loadHistoryForDate
content = content.replace(
  /if\s*\(\s*window\.historyMergedMode\s*\)\s*\{\s*window\.renderFanAssembleDimmerMergedForDate\(dateStr\);\s*return;\s*\}/,
...
// 3. updateMergedHistoryButtonState
content = content.replace(
  /function updateMergedHistoryButtonState\(\) \{[\s\S]*?btn\.classList\.toggle\('is-active', !!window\.historyMergedMode\);\s*\}/,
...
// 4. showFanAssembleDimmerMergedHistory Mode set
content = content.replace(
  /window\.historyMergedMode = !window\.historyMergedMode;\s*updateMergedHistoryButtonState\(\);\s*if \(!window\.historyMergedMode\) \{/,
...
// 5. renderFanAssembleDimmerMergedForDate Mode set
content = content.replace(
  /window\.historyMergedMode = true;\s*updateMergedHistoryButtonState\(\);/,
  `window.historyMergedMode = 'assemble_dimmer';
  updateMergedHistoryButtonState();`
);

// 7. Add new functions (inserting before collectFanAssembleDimmerTotals)
const rojShaplaCode = `
...
// 8. Update Excel export conditionals
content = content.replace(
  /const merged = collectFanAssembleDimmerTotals\(res\.state\);/g,
...
// 9. Update PDF Title
content = content.replace(
  /Worker Attendance Summary \(Fan Assemble \+ Fan Dimmer\)/g,
  `Worker Attendance Summary (' + (window.historyMergedMode === 'roj_shapla' ? 'Fan Rojonigondha + Fan Sada Shapla' : 'Fan Assemble + Fan Dimmer') + ')`
);
...
if (content === initialContent) {
  ...
  // also update fix_excel_style.js if it exists, since it has collectFanAssembleDimmerTotals
  if (fs.existsSync('fix_excel_style.js')) {
    let fixContent = fs.readFileSync('fix_excel_style.js', 'utf8');
    fixContent = fixContent.replace(
      /const merged = collectFanAssembleDimmerTotals\(res\.state\);/g,
      "const merged = window.historyMergedMode === 'roj_shapla' ? collectFanRojonigondhaShaplaTotals(res.state) : collectFanAssembleDimmerTotals(res.state);"
    );
    if (!fixContent.includes('function collectFanRojonigondhaShaplaTotals')) {
        fixContent = fixContent + "\\n" + rojShaplaCode;
    }
    fs.writeFileSync('fix_excel_style.js', fixContent);
  }
